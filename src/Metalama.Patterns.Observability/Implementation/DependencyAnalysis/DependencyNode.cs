...
/// using the <see cref="DependencyGraphExtensions.DuplicateUsing{TNode, TContext}(DependencyNode, TContext)"/> extension method.
...
/// </remarks>
[CompileTime]
internal sealed class DependencyNode : IGraphBuildingNode, IDependencyNode<DependencyNode>
{
    private readonly DependencyNode? _parent;
    private Dictionary<ISymbol, DependencyNode>? _children;
    private HashSet<DependencyNode>? _referencedBy;

    internal DependencyNode() { }

    private DependencyNode( DependencyNode parent, ISymbol symbol )
    {
        this._parent = parent;
        this.Symbol = symbol;
    ...
    public DependencyNode Parent => this._parent ?? throw new NotSupportedException( "Parent is not defined for a root node." );

    public bool HasChildren => this._children != null;

    public IReadOnlyCollection<DependencyNode> Children => (IReadOnlyCollection<DependencyNode>?) this._children?.Values ?? Array.Empty<DependencyNode>();

    public bool HasReferencedBy => this._referencedBy != null;

    public IReadOnlyCollection<DependencyNode> ReferencedBy => (IReadOnlyCollection<DependencyNode>?) this._referencedBy ?? Array.Empty<DependencyNode>();
    ...
    DependencyNode IGraphBuildingNode.GetOrAddChild( ISymbol childSymbol )
    {
        if ( childSymbol == null )
        {
        ...
        {
            this._children = new Dictionary<ISymbol, DependencyNode>( SymbolEqualityComparer.Default );
            result = new DependencyNode( this, childSymbol );
    ...
    void IGraphBuildingNode.AddReferencedBy( DependencyNode node )
    {
        if ( node == null )
        {
        ...
        this._referencedBy ??= new HashSet<DependencyNode>();
    ...
    public string ToString( DependencyNode? highlight )
    {
        var sb = new StringBuilder();
    ...
    private void ToString( StringBuilder appendTo, int indent, Func<DependencyNode, bool>? shouldHighlight = null )
    {
