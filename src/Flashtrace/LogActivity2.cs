...
namespace PostSharp.Patterns.Diagnostics
{
        ...
        where TActivityDescription : IMessage
    {
        ...
        private void SetOutcomeImpl<TMessage>(LogLevel level, in TMessage message, Exception exception, in CloseActivityOptions options, ref CallerInfo callerInfo, int skipFrames = 1)
            where TMessage : IMessage
        {
            ...
            IContextLocalLogger contextLocalLogger = this.Logger;
            ...
            try
            {
                if (this.Context == null)
                {
                    if ( contextLocalLogger.IsEnabled(level))
                    {
                        ...
                        using (ICustomLogRecordBuilder builder = contextLocalLogger.GetRecordBuilder(new CustomLogRecordOptions(level, LogRecordKind.CustomRecord, CustomLogRecordAttributes.WriteActivityDescriptionAndOutcome, options.Data), ref callerInfo))
                        {
                ...
                else
                {
                    try
                    {
                        if ( contextLocalLogger.IsEnabled(level))
                        {
                            ...
                            using (ICustomLogRecordBuilder builder = contextLocalLogger.GetRecordBuilder(new CustomLogRecordOptions(level, LogRecordKind.CustomActivityExit, CustomLogRecordAttributes.WriteActivityOutcome, options.Data), ref callerInfo, this.Context))
                            {
