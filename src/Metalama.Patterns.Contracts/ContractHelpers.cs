...
public static class ContractHelpers
{
    ...
    /// <returns>
    /// A tuple where: <c>IsInRange</c> is <see langword="true"/> if <paramref name="value"/> is of a supported type and
    /// is within the allowed range; <c>UnderlyingType</c> indicates the actual type of <paramref name="value"/>, or the
    /// underlying type if <paramref name="value"/> is nullable; and <c>IsNull</c> is <see langword="true"/> when
    /// <paramref name="value"/> matches <see langword="null"/>.
    /// </returns>
    internal static ValidateResult ValidateRange<T>(T value, in RangeValues rangeValues)
    {
    ...
    public static Regex GetRegex(string pattern, RegexOptions options) => _cachedRegexes.GetOrAdd(
        (pattern, options),
        x => new Regex(x.Pattern, x.Options));
    ...
    public static bool IsInRange<T>(
        ...
        double maxDouble,
        decimal minDecimal,
        decimal maxDecimal) => ValidateRange(
        value,
        new RangeValues(minInt64, maxInt64, minUInt64, maxUInt64, minDouble, maxDouble, minDecimal, maxDecimal)).IsInRange;
    ...
    public static Func<string?, bool> IsValidCreditCardNumber
    {
        get;
        set;
    } = value =>
...
#else
        var str2 = value.Replace("-", string.Empty).Replace(" ", string.Empty);
#endif
        var checksum = 0;
        var toggle = false;
