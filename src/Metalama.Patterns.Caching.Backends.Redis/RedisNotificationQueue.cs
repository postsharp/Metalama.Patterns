...
#pragma warning disable CA2213 // This is properly disposed
        private readonly BlockingCollection<RedisNotification> notificationQueue = new BlockingCollection<RedisNotification>();
        private readonly ManualResetEventSlim notificationProcessingLock = new ManualResetEventSlim(true);
#pragma warning restore CA2213

        private readonly Thread notificationProcessingThread;
        private readonly TaskCompletionSource<bool> notificationProcessingThreadCompleted = new TaskCompletionSource<bool>();
        private readonly ImmutableArray<RedisChannel> channels;
        private readonly Action<RedisNotification> handler;
        private readonly TimeSpan connectionTimeout;
        private volatile TaskCompletionSource<bool> queueEmptyTask = new TaskCompletionSource<bool>();
        ...
        private readonly StackTrace allocationStackTrace = new StackTrace();

        private RedisNotificationQueue(string id, IConnectionMultiplexer connection, ImmutableArray<RedisChannel> channels,
                                       Action<RedisNotification> handler, TimeSpan connectionTimeout)
        {
            this.id = id;
        ...
        ~RedisNotificationQueue()
        {
            try
            {
                this.logger.Error.Write( Formatted( "The resource {ObjectId} has not been properly disposed. Allocation stack trace:\n{AllocStackTrace}",
                                   this.id, this.allocationStackTrace.ToString()) );
        ...
        private void Init()
        {
            ...
            foreach (RedisChannel channel in this.channels)
            {
                ...
                {
                    this.logger.Debug.EnabledOrNull?.Write(Formatted("Not connected to {Channel}. Waiting {Delay} ms and retrying.", channel, connectDelay ));
        ...
        public static RedisNotificationQueue Create(string id, IConnectionMultiplexer connection, ImmutableArray<RedisChannel> channels,
                                                     Action<RedisNotification> handler, TimeSpan connectionTimeout)
        {

            RedisNotificationQueue queue = new RedisNotificationQueue(id, connection, channels, handler, connectionTimeout);
            queue.Init();
            return queue;
        ...
        [SuppressMessage("Microsoft.Reliability", "CA2000:Dispose objects before losing scope")]
        public static Task<RedisNotificationQueue> CreateAsync(string id, IConnectionMultiplexer connection, ImmutableArray<RedisChannel> channels, Action<RedisNotification> handler, TimeSpan connectionTimeout, CancellationToken cancellationToken)
        {
        ...
        private static void ProcessNotificationQueue(object state)
        {
            ...
            try
            {
                ...
                LogSource logger = queue.logger;
                string id = queue.id;
                BlockingCollection<RedisNotification> blockingCollection = queue.notificationQueue;

                logger.Debug.EnabledOrNull?.Write( Formatted( "The {ThreadName} thread for object {ObjectId} has started.", nameof(ProcessNotificationQueue), id ) );
                ...
                foreach ( RedisNotification notification in blockingCollection.GetConsumingEnumerable() )
                {
                    ...
                    try
                    {
                        ...
                        using ( var activity =
                            logger.Default.OpenActivity( Formatted("Processing notification {Value} received on channel {Channel}", notification.Value, notification.Channel )) )
                        {
                ...
                logger.Debug.EnabledOrNull?.Write( Formatted("The {ThreadName} thread for object {ObjectId} has completed.", nameof(ProcessNotificationQueue), id ));
