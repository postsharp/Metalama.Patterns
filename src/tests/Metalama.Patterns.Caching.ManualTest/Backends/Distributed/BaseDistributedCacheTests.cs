            ...
            try
            {
                ...
                for ( int j = 0; j < backends.Length; j++ )
                {
                    ...
                    for ( int i = 0; i < backends.Length; i++ )
                    {
                        ...
                        Assert.False( await backends[i].ContainsItemAsync( string.Format( "i{0}", i ) ),
                                        $"Backend {i} still contains the item but it was invalidated by backend {j}." );
                        Assert.False( await backends[i].ContainsDependencyAsync( dependencyKey ),
