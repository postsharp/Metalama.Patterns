...
namespace PostSharp.Patterns.Caching.Tests
{
    public class CacheLockTests : IDisposable
    {
        ...
        {
            Task t1 = Task.Run( () => this.TestLoop() );
            Task t2 = Task.Run(() => this.TestLoop());
        ...
        {
            Task t1 = Task.Run(this.TestLoopAsync);
            Task t2 = Task.Run(this.TestLoopAsync);
        ...
        public async Task TestAsyncLockTimeout()
        {
            ...
            AsyncBarrier barrier = new AsyncBarrier( 2 ); 
        ...
        public async Task TestAsyncLockTimeoutAsync()
        {
            ...
            Task delay = Task.Delay( globalTimeout );
            Task t = await Task.WhenAny( Task.WhenAll( t1, t2 ), delay );
        ...
        [Cache(ProfileName = "LocalLock")]
        private async Task<int> CachedMethodAsync([NotCacheKey] int sleepTime = 1, [NotCacheKey] AsyncBarrier barrier = null, [NotCacheKey] bool assert = true)
        {
            if ( assert )
            {
                Assert.Equal( 1, Interlocked.Increment( ref this.counter ) );
